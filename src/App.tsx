...
import { currentDeviceLoadDecision, currentWebGpuSupport } from "./utils/device";
...
export function App() {
  ...
  const [isLoadingMoreNextTokens, setIsLoadingMoreNextTokens] = useState(false);
  const promptRunId = useRef(0);
  const actionRunId = useRef(0);
  const continueRunId = useRef(0);
  const loadMoreRunId = useRef(0);
  ...
  useEffect(() => {
    ...
    const prepareModelLoad = async () => {
      const webGpuSupport = await currentWebGpuSupport();
      const stillIdle = useExplorerStore.getState().loadStatus === "idle";
      if (cancelled || !stillIdle) return;

      if (!webGpuSupport.isSupported) {
        setStatePatch({
          loadStatus: "error",
          error: webGpuSupport.reason,
        });
        return;
      }
  ...
  const loadMoreNextTokens = useCallback(async () => {
    const backend = backendRef.current;
    if (
      !backend ||
      state.isContinuing ||
    ...
    const runId = loadMoreRunId.current + 1;
    loadMoreRunId.current = runId;
    const tokenIds = [...state.tokenIds];
    const nextLimit = state.nextTokens.length + DEFAULT_TOP_N;

    setIsLoadingMoreNextTokens(true);
    setStatePatch({ isInferring: true, error: null });
    try {
      const nextTokens = await backend.getTopNextTokens(tokenIds, nextLimit);
      const latestState = useExplorerStore.getState();
      const isSameTokenContext =
        latestState.tokenIds.length === tokenIds.length &&
        latestState.tokenIds.every(
          (tokenId, index) => tokenId === tokenIds[index],
        );

      if (loadMoreRunId.current === runId && isSameTokenContext) {
        setStatePatch({ nextTokens });
      }
    } catch (error) {
      if (loadMoreRunId.current === runId) {
        setStatePatch({
          error: error instanceof Error ? error.message : String(error),
        });
      }
      ...
      if (loadMoreRunId.current === runId) {
        setStatePatch({ isInferring: false });
        setIsLoadingMoreNextTokens(false);
      }
    }
  }, [
    setStatePatch,
      ...
      >
        <div className="main-column">
          ...
          <NextTokenTable
            ...
            isLoadingMore={isLoadingMoreNextTokens}
            onAppend={appendCandidate}
            onLoadMore={loadMoreNextTokens}
          />
        </div>
